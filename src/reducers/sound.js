...
const playSound = id => state => state.map(s => s.id === id ? { ...s, play: true } : { ...s, play: false });
const playSounds = ids => state => state.map(s => ids.includes(s.id) ? { ...s, play: true } : { ...s, play: false });
